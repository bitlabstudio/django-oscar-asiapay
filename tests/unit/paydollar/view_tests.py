...
class MockedAsiaPayTests(TestCase):
...
class EdgeCaseTests(MockedAsiaPayTests):

    def setUp(self):
        self.client = Client()

    def test_empty_basket_shows_error(self):
        url = reverse('asiapay-redirect')
    ...
    def test_missing_shipping_address(self):
        from asiapay.paydollar.views import RedirectView
...
class RedirectToAsiaPayTests(MockedAsiaPayTests):
    ...
    def perform_action(self):
        self.add_product_to_basket()
        response = self.client.get(reverse('asiapay-redirect'))
        self.url = URL.from_string(response['Location'])

    def test_nonempty_basket_redirects_to_asiapay(self):
        self.assertEqual('www.sandbox.asiapay.com', self.url.host())
...
class FailedTxnTests(MockedAsiaPayTests):
    ...
    def perform_action(self):
        ...
        basket.freeze()
        url = reverse('asiapay-success-response',
    ...
    def test_context(self):
        self.assertTrue('asiapay_amount' not in self.response.context)


class PreviewOrderTests(MockedAsiaPayTests):
    ...
    def test_context(self):
        self.assertEqual(D('33.98'), self.response.context['asiapay_amount'])

    def test_keys_in_context(self):
        keys = ('shipping_address', 'shipping_method',
                'payer_id', 'token', 'asiapay_user_email',
                'asiapay_amount')
        for k in keys:
            self.assertTrue(k in self.response.context, "%s not in context" % k)


class SubmitOrderTests(MockedAsiaPayTests):
    ...
    def perform_action(self):
        ...
        basket = Basket.objects.all()[0]
        basket.freeze()
        url = reverse('asiapay-place-order', kwargs={'basket_id': basket.id})
    ...
    def patch_http_post(self, post):
        ...
        do_response = 'TOKEN=EC%2d6WY34243AN3588740&SUCCESSPAGEREDIRECTREQUESTED=false&TIMESTAMP=2012%2d04%2d19T10%3a07%3a47Z&CORRELATIONID=3db1d5276ddfd&ACK=Success&VERSION=88%2e0&BUILD=2808426&INSURANCEOPTIONSELECTED=false&SHIPPINGOPTIONISDEFAULT=false&PAYMENTINFO_0_TRANSACTIONID=51963679RW630412N&PAYMENTINFO_0_TRANSACTIONTYPE=paydollarcheckout&PAYMENTINFO_0_PAYMENTTYPE=instant&PAYMENTINFO_0_ORDERTIME=2012%2d04%2d19T09%3a42%3a50Z&PAYMENTINFO_0_AMT=33%2e98&PAYMENTINFO_0_FEEAMT=1%2e36&PAYMENTINFO_0_TAXAMT=0%2e00&PAYMENTINFO_0_CURRENCYCODE=GBP&PAYMENTINFO_0_PAYMENTSTATUS=Pending&PAYMENTINFO_0_PENDINGREASON=paymentreview&PAYMENTINFO_0_REASONCODE=None&PAYMENTINFO_0_PROTECTIONELIGIBILITY=Ineligible&PAYMENTINFO_0_PROTECTIONELIGIBILITYTYPE=None&PAYMENTINFO_0_SECUREMERCHANTACCOUNTID=YYH7BB4UHPKC4&PAYMENTINFO_0_ERRORCODE=0&PAYMENTINFO_0_ACK=Success'

        def side_effect(url, payload, **kwargs):
            if 'GetPaydollarCheckoutDetails' in payload:
                return self.get_mock_response(get_response)
            elif 'DoPaydollarCheckoutPayment' in payload:
...
class SubmitOrderErrorsTests(MockedAsiaPayTests):
